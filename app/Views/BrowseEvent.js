          ...
          <Text style={{marginVertical: 10, fontSize: 25, fontWeight: '500'}}>Services</Text>
            <View style={{marginTop: 10, justifyContent: 'center', alignItems: 'center', backgroundColor: '#F5F5F5'}}>
              <View style={{flexDirection: 'row', paddingTop: 40, paddingBottom: 15, paddingHorizontal: 40}}>
                <View>
                  <ZIcon photoUrlSelected={require('../Assets/Bartendericon.png')} photoUrlUnSelected={require('../Assets/Bartendericonselected.png')} isSelected={false} size={50} />
                  <Text style={{paddingLeft: 7, color: '#5E5CBD', fontWeight: '500'}}>Bartender</Text>
                </View>
                <View>
                  <ZIcon photoUrlSelected={require('../Assets/waitericon.png')} photoUrlUnSelected={require('../Assets/waitericonselected.png')} isSelected={false} size={50} />
                  <Text style={{paddingLeft: 7, color: '#5E5CBD', fontWeight: '500'}}>Waiter</Text>
                </View>
              </View>
            </View>
            <View style={{flex: 1, alignItems: 'flex-end'}}>
              <TouchableOpacity onPress={() => this.setState({modalVisible: false})} style={{position: 'absolute', bottom: 75, left: 65}}>
      ...
      return (
        <ScrollView style={styles.container} contentContainerStyle={[styles.scrollContent, {top: 0, paddingBottom: 50}]}>
          ...
          <View style={styles.body}>
            ...
            <ZSliderCard>
              <ZRoundedButton name="Events near you" isSelected={this.state.selected} selectedColor="#5F5FBA" selectedButton={(value) => this.getFilterBy('Events near you', 0, value)} />
              <ZRoundedButton name="Upcoming Events" isSelected={this.state.selected} selectedColor="#5F5FBA" selectedButton={(value) => this.getFilterBy('Upcoming Events', 1, value)} />
              <ZRoundedButton name="Positions needed" isSelected={this.state.selected} selectedColor="#5F5FBA" selectedButton={(value) => this.getFilterBy('Positions needed', 2, value)} />
            </ZSliderCard>
            <ZSliderCard>
              <ZRoundedButton name="All" isSelected={this.state.selected} selectedColor="#64DAE7" height={35} selectedButton={(value) => this.getFilterBy('All', 3, value)} />
              <ZRoundedButton name="Birthday" isSelected={this.state.selected} selectedColor="#64DAE7" height={35} selectedButton={(value) => this.getFilterBy('Birthday', 4, value)} />
              <ZRoundedButton name="Wedding" isSelected={this.state.selected} selectedColor="#64DAE7" height={35} selectedButton={(value) => this.getFilterBy('Wedding', 5, value)} />
              <ZRoundedButton name="Conference" isSelected={this.state.selected} selectedColor="#64DAE7" height={35} selectedButton={(value) => this.getFilterBy('Conference', 6, value)} />
              <ZRoundedButton name="Music Festival" isSelected={this.state.selected} selectedColor="#64DAE7" height={35} selectedButton={(value) => this.getFilterBy('Music Festival', 7, value)} />
              <ZRoundedButton name="Family Event" isSelected={this.state.selected} selectedColor="#64DAE7" height={35} selectedButton={(value) => this.getFilterBy('Family Event', 8, value)} />
              <ZRoundedButton name="Other" isSelected={this.state.selected} selectedColor="#64DAE7" height={35} selectedButton={(value) => this.getFilterBy('Other', 9, value)} />
            </ZSliderCard>
          </View>
          {this.renderPeopleNeededModal()}
          {
            ...
            return (
              <View key={id} style={styles.body}>
                <ZEventCard event={data.name} image={data.image} type={null} interested={true} skills={true} exp={false} job={date} jobType={time} rsa={null} location={null} onPress={()=>navigate('', { name: '' })} isInterested={isInterested} onPressInterested={()=> this.onSelectInterested(data._id)} onPressEvent={()=> this.setState({modalVisible: true})} />
              </View>
            )
          })
        }
