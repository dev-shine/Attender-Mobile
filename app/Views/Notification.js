    ...
    return{
      headerStyle: {
        ...
        <TouchableOpacity onPress={()=>[navigation.state.params.onGoBack(), navigation.goBack()]}>
          <Icon name="md-arrow-back" size={25} color="white" style={{backgroundColor: 'transparent', marginLeft: 20}} />
        </TouchableOpacity>
        )
    }
  }
    ...
    if (this.state.isStaffView) {
      return(
        ...
        >
          ...
          <View style={{flex: 1}}>
            <View style={{position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, opacity: 0.9, backgroundColor: 'black'}} />

            <View style={{flex: 1}}>
              <View style={{alignItems: 'center', justifyContent: 'center'}}>
                <View style={{backgroundColor: 'white', width: '90%', height: '80%', marginTop: 50, borderRadius: 15}}>
                  ...
                  <View style={{alignItems: 'center'}}>
                    <Image style={{width: '80%', height: 180}} source={{uri: res.employer.image || 'http://www.theemailcompany.com/wp-content/uploads/2016/02/no-image-placeholder-big-300x200.jpg'}}/>
                    ...
                    <View style={{flexDirection: 'row', justifyContent: 'center', alignItems: 'center', borderBottomWidth: 0.5, borderColor: '#E9E9E9', paddingBottom: 20}}>
                      <View style={{flexDirection: 'row', marginVertical: 5, justifyContent: 'center', alignItems: 'center', marginRight: 10}}>
                        ...
                        <Text style={{color: '#7C7885', fontSize: 14, fontWeight: '500'}}>{res.employer ? res.employer.tag1 : ''}</Text>
                      </View>
                      <View style={{flexDirection: 'row', marginVertical: 5, justifyContent: 'center', alignItems: 'center'}}>
                        <Icon name="ios-radio-button-on" size={10} color="cyan" style={{marginRight: 10}}/>
                        <Text style={{color: '#7C7885', fontSize: 14, fontWeight: '500'}}>{res.employer ? res.employer.tag2 : ''}</Text>
                      </View>
                    </View>
                    <ZCard>
            ...
            <View style={{flex: 1}}>
              <View style={{alignItems: 'center', justifyContent: 'center'}}>
                <View style={{backgroundColor: 'white', width: '90%', height: '80%', marginTop: 50, borderRadius: 15}}>
                  ...
                  <View style={{alignItems: 'center'}}>
                    <Image style={{width: 100, height: 100, borderRadius: 50, borderWidth: 1, borderColor: 'white'}} source={{uri: res.staffId.avatar || 'https://www.nztcc.org/themes/kos/images/avatar.png'}}/>
                    <ZHero text={res.staffId.fullname} styles={{color: '#33314B'}} />
                    <View style={{alignItems: 'center'}}>
                      <Text style={{fontSize: 14, marginVertical: 5}}>{res.staffId.position.join(' | ')} <Text style={{fontSize: 14, color: '#78757E'}}>{res.staffId.frequency}</Text></Text>
                      <NRater />
                      <Text style={{fontSize: 14}}>RSA Cerftified</Text>
                      <Text style={{fontSize: 14, color: '#78757E'}}>{res.staffId.description.join(' / ')}</Text>
                    </View>
                  </View>
                </View>
              </View>
      ...
      const rightButtons = ($id) => {
        return [
          <TouchableOpacity onPress={()=>this.setState({isStaffView: true})}>
            <View style={{backgroundColor: '#6E6F92', width: 120, height: 80, padding: 20, justifyContent: 'center', alignItems: 'center'}}>
          ...
          <TouchableOpacity onPress={()=>this.onDeleteNotification($id)}>
            <View style={{backgroundColor: '#F00010', width: 120, height: 80, padding: 20, justifyContent: 'center', alignItems: 'center'}}>
        ...
        <ScrollView>
          {
                      ...
                      <View>
                          <View style={{flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', marginVertical: 15, marginHorizontal: 10}}>
                            <View style={{paddingRight: 10}}>
                              <Image style={{width: 50, height: 50, borderRadius: 25, borderWidth: 1, borderColor: 'white'}} source={{uri: res.staffId.avatar}}/>
                            </View>
                            <View style={{flex: 1}}>
                              <Text>
                                <Text style={{fontSize: 14, fontWeight: '500'}}>{res.staffId.fullname}</Text>
                                <Text style={{fontSize: 12, color: '#8C8C8C'}}> {status} </Text>
                                <Text style={{fontSize: 12, color: '#555DB9'}}>{res.employer.name}.</Text>
                                <Text style={{fontSize: 10, color: '#8C8C8C', marginHorizontal: 10}}>  {moment(res.createdAt).fromNow()}</Text>
                              </Text>
                            </View>
                          </View>
                      </View>
            ...
            <View style={{flex: 1, alignItems: 'center', marginTop: 20}}>
              <Text style={{fontSize: 14, fontWeight: '500', color: '#C5C5C5'}}>You have 0 Notifications.</Text>
            </View>
          }
        </ScrollView>
      )
