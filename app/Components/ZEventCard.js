...
export default class ZEventCard extends Component {
    ...
    renderLocation = () => {
        return (<Text style={{fontSize: 13, color: '#363346'}}> <Image style={{width: 11, height: 14}}
                                                                       source={{uri: 'https://upload.wikimedia.org/wikipedia/commons/thumb/f/fe/Media_Viewer_Icon_-_Location.svg/1000px-Media_Viewer_Icon_-_Location.svg.png'}}/> {this.props.location}
        </Text>)
    }
    renderDetails = () => {
        return (
            <View>
                ...
                <Text style={{fontSize: 11, color: '#8F8F8F'}}>{this.props.job} <Text
                    style={{fontSize: 11, color: '#8F8F8F'}}>{this.props.jobType}</Text></Text>
                <Text style={{fontSize: 11, color: '#8F8F8F', paddingVertical: 3}}>Venue: <Text
                    style={{fontSize: 11, color: '#78757E', paddingVertical: 3}}>{this.props.rsa}</Text></Text>
            </View>
        )
    }
    ...
    renderSkill = () => {
        if (this.props.skills) {
            return (
                <View style={{flex: 1}}>
                    <ZSliderCard>
                        <Text style={{fontSize: 16, color: '#78757E', alignSelf: 'center', marginRight: 10}}>Position
                            Needed :</Text>
                        <ZIcon photoUrlSelected={require('../Assets/Bartendericon.png')}
                               photoUrlUnSelected={require('../Assets/Bartendericon.png')} isSelected={false}
                               small={true} selectedIcon={this.props.onPressEvent}/>
                        <ZIcon photoUrlSelected={require('../Assets/waitericon.png')}
                               photoUrlUnSelected={require('../Assets/waitericon.png')} isSelected={false} small={true}
                               selectedIcon={this.props.onPressEvent}/>
                    </ZSliderCard>
                </View>
            )
        }
